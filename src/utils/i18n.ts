...
const translations: Record<string, Record<AppLanguage, string>> = {
  ...
  'form.errTitleRequired': { en: 'Recipe title is required', it: 'Il titolo della ricetta e obbligatorio', es: 'El titulo de la receta es obligatorio', fr: 'Le titre de la recette est requis', de: 'Rezeptname ist erforderlich', pt: 'O titulo da receita e obrigatorio', ja: 'レシピ名は必須です', zh: '食谱标题为必填项', ko: '레시피 제목은 필수입니다', ru: 'Название рецепта обязательно' },
  'form.errIngredient': { en: 'Add at least one ingredient with a name', it: 'Aggiungi almeno un ingrediente con un nome', es: 'Anade al menos un ingrediente con nombre', fr: 'Ajoutez au moins un ingredient avec un nom', de: 'Fugen Sie mindestens eine Zutat mit Namen hinzu', pt: 'Adicione pelo menos um ingrediente com nome', ja: '名前付きの材料を1つ以上追加してください', zh: '请至少添加一个有名称的食材', ko: '이름이 있는 재료를 하나 이상 추가하세요', ru: 'Добавьте хотя бы один ингредиент с названием' },
  'form.errStep': { en: 'Add at least one step with instructions', it: 'Aggiungi almeno un passaggio con istruzioni', es: 'Anade al menos un paso con instrucciones', fr: 'Ajoutez au moins une etape avec des instructions', de: 'Fugen Sie mindestens einen Schritt mit Anleitung hinzu', pt: 'Adicione pelo menos um passo com instrucoes', ja: '手順のあるステップを1つ以上追加してください', zh: '请至少添加一个有说明的步骤', ko: '설명이 있는 단계를 하나 이상 추가하세요', ru: 'Добавьте хотя бы один шаг с инструкциями' },
  ...
  'settings.errKeyRequired': { en: 'Enter an API key for the selected provider', it: 'Inserisci una chiave API per il provider selezionato', es: 'Introduce una clave API para el proveedor seleccionado', fr: 'Saisissez une cle API pour le fournisseur selectionne', de: 'Geben Sie einen API-Schlussel fur den gewahlten Anbieter ein', pt: 'Insira uma chave API para o provedor selecionado', ja: '選択したプロバイダーのAPIキーを入力してください', zh: '请输入所选提供商的API密钥', ko: '선택한 제공자의 API 키를 입력하세요', ru: 'Введите API-ключ для выбранного провайдера' },
  ...
  'ai.urlPlaceholder': { en: 'Paste a recipe URL…', it: 'Incolla l\'URL di una ricetta…', es: 'Pega la URL de una receta…', fr: 'Collez l\'URL d\'une recette…', de: 'Rezept-URL einfügen…', pt: 'Cole a URL de uma receita…', ja: 'レシピのURLを貼り付け…', zh: '粘贴食谱链接…', ko: '레시피 URL 붙여넣기…', ru: 'Вставьте URL рецепта…' },
